...
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    const setMode = (mode: "idle" | "link" | "label", text = "") => {
      ...
      if (mode === "idle") {
        gsap.to(ring.current, { width: 38, height: 38, backgroundColor: "rgba(86,216,255,0)", borderColor: "rgba(220,231,248,0.4)", duration: 0.45, ease: "expo.out" });
        gsap.to(dot.current, { scale: 1, duration: 0.3 });
        gsap.to(label.current, { opacity: 0, duration: 0.15 });
      } else if (mode === "link") {
        gsap.to(ring.current, { width: 64, height: 64, backgroundColor: "rgba(86,216,255,0.10)", borderColor: "rgba(86,216,255,0.9)", duration: 0.45, ease: "expo.out" });
        gsap.to(dot.current, { scale: 0.4, duration: 0.3 });
        gsap.to(label.current, { opacity: 0, duration: 0.15 });
      } else {
        gsap.to(ring.current, { width: 96, height: 96, backgroundColor: "rgba(86,216,255,0.95)", borderColor: "rgba(86,216,255,1)", duration: 0.5, ease: "expo.out" });
  ...
  return (
    <>
      <div
        ref={ring}
        aria-hidden
        className="pointer-events-none fixed left-0 top-0 z-[9998] flex items-center justify-center rounded-[3px] border backdrop-blur-[1px]"
        style={{ width: 38, height: 38, borderColor: "rgba(220,231,248,0.4)", opacity: 0 }}
      >
        <span ref={label} className="font-mono text-[0.62rem] font-medium uppercase tracking-[0.14em] text-[#04121a] opacity-0" />
      </div>
      <div
        ref={dot}
        aria-hidden
